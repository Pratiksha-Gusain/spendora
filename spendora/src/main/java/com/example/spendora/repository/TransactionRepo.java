...
public interface TransactionRepo extends CrudRepository<Transaction, Long>, JpaSpecificationExecutor<Transaction> {
    ...
    @Query("SELECT t.transactionDate as transactionDate, " +
            "SUM(CASE WHEN t.type = com.example.spendora.model.TransactionType.INCOME THEN t.amount ELSE 0.0 END) as income, " +
            "SUM(CASE WHEN t.type = com.example.spendora.model.TransactionType.EXPENSE THEN t.amount ELSE 0.0 END) as expense " +
            "FROM Transaction t " +
    ...
    @Query("SELECT COALESCE(t.userCategory.name, t.systemCategory.name, 'Uncategorized') as label, SUM(ABS(t.amount)) as amount " +
            "FROM Transaction t " +
            "WHERE t.appUser.id = :appUserId AND t.type = com.example.spendora.model.TransactionType.EXPENSE " +
